...
- **aorch 결함(이월)**: (1) Claude 워커의 비-한도 실패(신뢰 다이얼로그 거부 등 exit 1)를 provider 한도로 자동 감지해 1시간 라우팅 차단 — stderr 패턴을 보고 한도일 때만 기록해야 함. (2) git 디렉터리가 별도 경로인 저장소(OneDrive 완화책 `git-meta`)에서는 Claude CLI 신뢰가 그 경로 기준이라 워크트리 dispatch 전에 `~/.claude.json` 신뢰 항목 확인 필요 — 런북 §6 함정에 추가할 것. (3) 게이트 명령을 태스크 범위와 맞춰야 한다(D1 실수).

## 갭 적용 (2026-09-03 밤)

- 사용자 선택 14건: MIS 문서 정정 8건(01·02·05·06·07·08·09·10) + MIS-03·04, LINK-01(킬스위치·재개 UI)·02·03(API 스모크 operator 섹션)·04, STUB-06, MAN-04, MAN-01(체크리스트 근거만, 서명 금지), STUB-01(리드 도그푸딩). 제외: STUB-02·03·04·05, MAN-02·03·05(실 키·라이브·승인 기준 등 사람 결정).
- 플랜 `scratchpad/plan-mm-apply.json`: D1(STATUS·매트릭스·체크리스트), D2(런북), C1(웹 킬스위치·재개 버튼 + /jobs '활동 로그'), C2(API run-smoke operator 섹션 공유 함수), R1 opus 리뷰. QuantPilot 워크트리 `.claude/worktrees/model-upgrade-fixes`를 브랜치 `claude/manager-map-gaps-2026-09-03`(main b528c88)으로 전환해 재사용(node_modules 복사본 유지). dry-run: 워커 4개 전부 Codex terra max(관측 기반), R1 opus xhigh. 21:34 KST 기동.
- **STUB-01 도그푸딩(리드, 메인 체크아웃, 프로세스 환경변수만)**: 기본 플래그 스모크는 `broker=mock, live_trading_enabled=false, operator blocked`. `FULLY_AUTOMATED_OPERATOR_ENABLED=true`로 켜자 `run_smoke`가 오퍼레이터 이전에 `validate_generic_runtime_environment()`에서 `generic_runtime_rejects_paper_arming_environment`로 기동 거부(dependencies.py:56) — 09-02 보안 라운드 Z1의 의도된 동작. 따라서 "플래그 잠김"의 실제 뜻은 "일반 런타임에서는 켤 수 없음; Level 5 도그푸딩은 `QUANTPILOT_RUNTIME_ROLE=paper-session` 잡 경로에서만". STATUS의 Level 3~5 행 비고를 이 사실로 갱신해야 한다(D1 범위 밖이라 run 종료 후 리드가 추가). 증거 `mm-evidence/stub01-smoke-flag-on.log`.
- 적용 run 1(21:34~21:47 KST): D1(terra max) 문서 3개 정정 완료 — 체크박스 0개 변경, STATUS에 API 전용 라우트 표(9개)·잡 실행 방식 표(6개)·professional 상태 행, 매트릭스 앵커 3건·부재 문서 참조 교체, 체크리스트 7항목 근거 줄. **게이트는 리드 실수로 빨간 불**: D1 검증 명령에 런북 조건(MIS-03 등, D1 범위 밖)을 넣어 둠 → change guard 통과·내용 확인 후 채택, D2 검증을 런북 조건만으로 분리해 재dispatch.
- 적용 run 2(21:48~22:24 KST, terra max): D2 런북 5건 완료(게이트 통과), C1 웹 킬스위치·재개 버튼(확인 게이트, 재개는 paused 시만)·/jobs '활동 로그'·operator.test 추가 — 샌드박스 Vite ACL로 vitest 미실행(이미 문서화된 함정) → 리드 게이트 vitest 31·빌드 통과, C2 API run-smoke와 CLI 잡이 공유 조립 함수로 통합 + 단위 테스트 — 리드 게이트 pytest exit 0. R1(opus)은 **Claude CLI가 워크트리를 '신뢰되지 않은 작업 공간'으로 거부**(QuantPilot의 git 디렉터리가 OneDrive 밖 `~/.local/git-meta/jusik.git`이라 그 경로에 신뢰 승인 없음) → `~/.claude.json`의 해당 항목 `hasTrustDialogAccepted: true`로 수정. 재dispatch는 "No eligible route": **aorch가 워커 exit 1을 anthropic 사용량 한도로 오인해 1시간 제한을 자동 기록**(`limits` auto-detect) → `limits clear anthropic` 후 Fable 핀으로 R1b 기동(22:38 KST).
- 리드가 STATUS Level 3·4·5 행 비고에 STUB-01 실측(일반 런타임은 arming 거부, paper-session 잡 경로만)을 추가.
- R1 세 번째 시도(09-04 06:34~06:48 KST, Fable high): **병합 안전** — 반증 5개 중 4개 held(안전 기본값 무변경, 킬스위치 2단계 확인·재개 paused 게이트, API 스모크 읽기 전용·CLI 본문 동일, 체크박스 0), 앵커 22개 대조 중 문서 2줄 불일치(F1 매트릭스 인용 제목, F2 STATUS의 '후속 범위' 문장이 같은 diff의 rename과 모순) → 리드 수정. F3 settings.json 훅 블록은 원래 커밋 제외. F4(low/standard) 재개 게이트가 모든 정책의 paused를 합산 → 요청 대상 정책 기준으로 리드 수정, tsc·vitest·빌드 재확인. 리뷰어 잔여 위험: 서버 resume이 paused 상태를 검사하지 않음(기존 동작), 설치 스캐폴딩(.claude/agents 등)은 커밋 대상 아님.
- 두 번째 R1 실패의 실체: 워커 stderr가 aorch `RATE_LIMIT_PATTERN`(rate limit/429/overloaded)에 걸려 한도로 기록 → Anthropic 과부하 응답이었을 가능성. 세 번째 시도는 정상. `limits clear anthropic`이 복구 수단.
