...
- ~~분해기(선행연구 미해결 + Claude 네이티브에 올라탐).~~ **2026-08-16 폐기** — 분해는 여전히 리드가 하되 그 결과의 형식을 aorch가 스키마로 강제하고 분배까지 책임진다. 즉 "분해기를 짓는 비용"은 그대로 피하면서 이 문서 §3.1이 스스로 지목한 "권고지 강제가 아니다"라는 약점만 제거했다. 근거와 설계는 `docs/plans/2026-08-16-decompose-dispatch-pipeline.md` 참조.
