...
- 스크립트(`pdf2md.py`): 페이지 PNG 렌더링, 썸네일 시트, 상태·재개, JSON 스키마·코드 검사, 도형 크롭, 결정론적 조립, 후처리·품질 감사.
...
- 문제문의 첫 줄에 `$$`가 있으면 검사 오류다(행렬은 둘째 줄부터). 문제 노트의 제목은 은행 번호 `### NNN.` 단독 헤더이며, 본문 첫 줄은 일반 문단으로 배치되어 수식이나 긴 문장이 제목에 갇히는 헤더 트랩(header-trapped)을 방지한다.
...
assemble은 제목·본문을 분리하고 NFC 정규화, 품질 감사와 메타데이터 기록을 수행한다. 수식의 의미를 추정하는 치환과 PUA 문자 삭제는 기본 실행에서 하지 않는다. 의심 항목은 원문을 유지한 채 검토 대상으로 보고한다.

과거 OCR 교정을 명시적으로 재현해야 할 때만 assemble <슬러그> --legacy-ocr-fixes 또는 postprocess <슬러그> --legacy-ocr-fixes를 사용한다. 3n+1을 3^{n+1}로 바꾸거나 \\sum(t)를 X(t)로 바꾸는 규칙은 원문과 다를 수 있다. 이 옵션으로 바뀐 텍스트는 실행 폴더의 legacy-ocr-fixes.jsonl에 변경 전후를 기록한다. 검사 통과를 위해 이 옵션을 자동으로 켜지 않는다.

`index.md`의 `## 텍스트 품질 요약`·`## 검토 권장`·`## 해설에서 만든 문제`·`## 미매칭 해설`·`## 고아 조각`·`## 실패·건너뜀 페이지`·`## 그림 미추출` 절을 요약해 보고한다. 문제 수, 해설 매칭 수, 품질 등급별 문항 수(good/poor/broken), 검토 권장 건수, 건너뛴 페이지와 이유를 적는다. 출력은 재생성되며 편집은 볼트로 옮긴 뒤에 한다는 점을 알린다.

문제 노트 `problems/NNN-<슬러그>.md`의 frontmatter: `source`, `pages`, `source_number`(인쇄 번호), `tags`, `points`(배점), `importance`(같은 시험지 묶음 안에서 배점 비중 × 문제 수, 1.0이 평균), `has_solution`, `solution_pages`, `from_solution`(해설에서 만든 문제), `confidence`, `text_quality`, `instruction_lost`. 제목은 은행 번호 `### NNN.`이며 배점 라벨은 제목·본문에서 빠진다. 시험 이름·장 이름은 노트에 쓰지 않는다(전체 md에는 인쇄된 heading이 그대로 남는다).

## 6. (선택) 사후 품질 감사 및 재정제

기존에 조립된 슬러그 디렉터리를 점검하거나 일괄 수선할 때 사용한다:

```powershell
pdf2md audit <슬러그>          # 텍스트 품질, PUA, 제어문자, 수식 문법 결함 검사 및 리포트 (수정 없음)
pdf2md postprocess <슬러그>    # 디렉터리 내 마크다운 일괄 정제·헤더 분리·품질 재평가
```

## 7. 여러 PDF를 Antigravity 헤드리스로 일괄 처리
