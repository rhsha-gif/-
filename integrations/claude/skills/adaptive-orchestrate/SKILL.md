...
7. Treat the worker receipt as a claim, and verify it yourself. **`aorch exec` does not run the task's verification commands and does not compare the claimed diff against the real one.** Run the task's verification commands, read the actual diff, and confirm the claimed files are the changed files. Run the tests — do not accept an unverified claim.
