...
`apply`는 가드형 실행자입니다. **`--approved` 없이는 어떤 git 쓰기도 하지 않고 계획만 출력**합니다(딸깍 전 미리보기). 한 번의 승인이 표시된 계획 전체를 실행하며, push·merge·머지된 브랜치 삭제도 포함합니다. 단, **미머지 브랜치 삭제는 미푸시 작업 소실 위험 때문에 별도의 `--confirm-unmerged`** 를 요구합니다. `finish`는 작업의 verify 게이트를 다시 돌려 녹색일 때만 main에 merge 커밋 후 push하고 머지된 브랜치를 삭제합니다(되돌리기용 pre-merge SHA 기록). verify 명령(`config.branch.verificationCommands`)이 없으면 게이트는 공허하게 통과하므로, 게이트를 실제로 걸려면 명령을 설정해야 합니다. main보다 behind인(다이버전) 브랜치는 finish 대신 sync를 먼저 권하며, 만약 finish 중 merge 충돌이 나면 `merge --abort`로 트리를 되돌리고 중단합니다. PR 관행 저장소면 status가 `repoIntegration: "pr"`로 알리고, 이 버전의 `finish`는 direct 머지 경로만 자동화합니다 — PR 생성은 호스트가 안내(후속 예정). 브랜치 이름은 호스트가 정합니다(aorch는 이름 규칙 config를 두지 않음). 설정은 선택적 `config.branch`(mainBranch/staleDays/integration/verificationCommands)로 조정합니다.
